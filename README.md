...
- **Progression chart** — DEFAULTS to the ratio-scale era (the comparable climb): every logged attempt of ratio rounds as a scatter (x = global iteration index), the **Pareto frontier** over valid ratio attempts as the headline step line, policy markers; legacy raw-probe rounds are toggled in with "include legacy probes" (their inclusion stretches the y-domain — off by default); hover per subpoint (round, node, mechanism, score, valid/failClass);
