...
- **Progression chart** — best score over rounds as inline SVG: the per-round series, the monotone running-best climb curve, a dashed **AlphaEvolve 2.635** reference line (the crossing), and policy-change markers where the active version changed; hover tooltips per round (bestScore, policyVersion, nodes);
