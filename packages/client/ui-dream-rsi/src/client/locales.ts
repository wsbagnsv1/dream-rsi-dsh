/**
 * `dreamRsi` namespace dictionaries, and the namespace's declaration.
 *
 * The namespace merge lives with its key set so that any module naming
 * `TranslateNS<'dreamRsi'>` needs only this file. Simplified Chinese is the
 * key-set source of truth, matching the DSH client convention.
 */
import type {} from '@deepseek-ai/dsh-client-ui-slots'

declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface LocaleNamespaceMap {
    /** Dream-RSI campaign dashboard copy. */
    dreamRsi: DreamRsiKey
  }
}

/** Simplified Chinese dictionary and key-set source of truth. */
export const zh = {
  'type.label': 'Dream-RSI',
  'guide.title': 'Dream-RSI 战役面板',
  'guide.description': '查看 .dreamrsi/ 发现库的实时状态',
  loading: '正在读取 .dreamrsi/ …',
  refreshedAt: '更新于 {time}',
  refresh: '刷新',
  'empty.title': '还没有 .dreamrsi/ 发现库',
  'empty.hint': '在 Dream-RSI 预设的会话里运行 dreamrsi_begin_round 后，这里会显示战役的实时状态。',
  'error.title': '读取失败',
  'error.retry': '重试',
  'champion.title': '当前最优',
  'champion.bestScore': '最佳分数',
  'champion.round': '来自轮次',
  'champion.policy': '打包程序',
  'champion.activePolicy': '现行策略',
  'champion.kind': '类型',
  'champion.none': '还没有已评测的轮次。',
  'champion.ratioNone': '没有比值尺度的轮次（早期探针轮次见下方标注）。',
  'champion.rawLegacy': '早期探针时代（原始分数，不可比）：{score}（{round}）',
  'champion.dreams': '做梦次数',
  'champion.rounds': '轮次',
  'champion.nodes': '节点',
  'lineage.title': '策略演化（v0001 → 现行）',
  'lineage.empty': '还没有策略版本。',
  'lineage.active': '现行',
  'lineage.retired': '退役',
  'lineage.parent': '由 {parent} 派生',
  'lineage.root': '初始版本',
  'rounds.title': '轮次',
  'rounds.empty': '还没有轮次。',
  'rounds.round': '轮次',
  'rounds.status': '状态',
  'rounds.policy': '策略',
  'rounds.nodes': '节点',
  'rounds.attempts': '尝试',
  'rounds.best': '最佳分数',
  'rounds.open': '进行中',
  'rounds.closed': '已关闭',
  'rounds.summary': '摘要',
  'dreams.title': '做梦报告',
  'dreams.empty': '还没有做梦报告。',
  'dreams.selected': '选中候选',
  'dreams.validWorlds': '有效世界',
  'dreams.noRegression': '无回退守卫',
  'dreams.floored': '触底',
  'dreams.score': '均分',
  'events.title': '最近事件',
  'events.empty': '还没有事件。',
  'tree.title': '发现树',
  'tree.loading': '正在读取节点…',
  'tree.empty': '这一轮还没有节点。',
  'tree.showBestPath': '高亮最优路径',
  'tree.bestPath': '最优路径',
  'tree.unscored': '未评测',
  'tree.truncated': '节点过多，只显示前 {count} 个。',
  'tree.select': '选择轮次',
  'forest.title': '发现森林',
  'forest.all': '全部轮次（森林视图）',
  'forest.bestPath': '全局最优路径（跨轮冠军谱系）',
  'forest.scoreGradient': '分数梯度',
  'forest.nodeCount': '{count} 个节点',
  'progress.title': '迭代爬升（全部尝试）',
  'progress.pareto': 'Pareto 前沿（目前最优）',
  'progress.valid': '有效',
  'progress.failed': '失败',
  'progress.attempts': '次尝试',
  'progress.markers': '策略切换',
  'progress.includeLegacy': '包含早期探针轮次（原始分数尺度）',
  'progress.erasNote': '不同目标时代的分数不可比：ratio = sum_radii / 2.635；>10 视为早期原始分数。',
  'progress.ratioOnlyEmpty': '没有比值尺度的轮次（勾选上面的开关可查看早期探针轮次）。',
  'progress.range': '绘图范围',
  'progress.rangeFrom': '起始迭代',
  'progress.rangeTo': '结束迭代',
  'progress.rangeReset': '重置',
  'progress.championLineage': '冠军谱系（保持最优的节点）',
  'progress.championHere': '此迭代保持冠军',
  'forest.star': '最优节点：{round} / {node}',
  'rounds.scaleRatio': 'ratio',
  'rounds.scaleRaw': 'raw·probe',
  'worlds': '{count} / {total}',
  'truncatedRounds': '部分轮次未列出（目录列表被截断，已读 {count} 个）。',
  'truncatedDreams': '只显示最近 {count} 份报告。',
} satisfies Record<string, string>

/** Dream-RSI dashboard dictionary key union. */
export type DreamRsiKey = keyof typeof zh

/** English dictionary, checked against the Chinese key set. */
export const en = {
  'type.label': 'Dream-RSI',
  'guide.title': 'Dream-RSI campaign dashboard',
  'guide.description': 'Live state of the .dreamrsi/ discovery store',
  loading: 'Reading .dreamrsi/…',
  refreshedAt: 'Updated {time}',
  refresh: 'Refresh',
  'empty.title': 'No .dreamrsi/ discovery store yet',
  'empty.hint': 'Run dreamrsi_begin_round in a session on the Dream-RSI preset, and the campaign state appears here.',
  'error.title': 'Read failed',
  'error.retry': 'Retry',
  'champion.title': 'Champion',
  'champion.bestScore': 'Best score',
  'champion.round': 'From round',
  'champion.policy': 'Packaged program',
  'champion.activePolicy': 'Active policy',
  'champion.kind': 'Kind',
  'champion.none': 'No evaluated round yet.',
  'champion.ratioNone': 'No ratio-scale rounds (the legacy probe rounds are tagged below).',
  'champion.rawLegacy': 'Legacy probe era (raw scale, not comparable): {score} ({round})',
  'champion.dreams': 'Dreams',
  'champion.rounds': 'Rounds',
  'champion.nodes': 'Nodes',
  'lineage.title': 'Policy lineage (v0001 → active)',
  'lineage.empty': 'No policy versions yet.',
  'lineage.active': 'active',
  'lineage.retired': 'retired',
  'lineage.parent': 'from {parent}',
  'lineage.root': 'bootstrap',
  'rounds.title': 'Rounds',
  'rounds.empty': 'No rounds yet.',
  'rounds.round': 'Round',
  'rounds.status': 'Status',
  'rounds.policy': 'Policy',
  'rounds.nodes': 'Nodes',
  'rounds.attempts': 'Attempts',
  'rounds.best': 'Best score',
  'rounds.open': 'open',
  'rounds.closed': 'closed',
  'rounds.summary': 'Summary',
  'dreams.title': 'Dream reports',
  'dreams.empty': 'No dream reports yet.',
  'dreams.selected': 'Selected candidate',
  'dreams.validWorlds': 'Valid worlds',
  'dreams.noRegression': 'No-regression guard',
  'dreams.floored': 'floored',
  'dreams.score': 'Mean score',
  'events.title': 'Recent events',
  'events.empty': 'No events yet.',
  'tree.title': 'Discovery tree',
  'tree.loading': 'Reading nodes…',
  'tree.empty': 'No nodes in this round.',
  'tree.showBestPath': 'Highlight best path',
  'tree.bestPath': 'Best path',
  'tree.unscored': 'unscored',
  'tree.truncated': 'Too many nodes, showing the first {count}.',
  'tree.select': 'Select round',
  'forest.title': 'Discovery forest',
  'forest.all': 'All rounds (forest view)',
  'forest.bestPath': 'Global best path (champion lineage across rounds)',
  'forest.scoreGradient': 'score gradient',
  'forest.nodeCount': '{count} nodes',
  'progress.title': 'Iteration climb (all attempts)',
  'progress.pareto': 'Pareto frontier (best so far)',
  'progress.valid': 'valid',
  'progress.failed': 'failed',
  'progress.attempts': 'attempts',
  'progress.markers': 'Policy changes',
  'progress.includeLegacy': 'Include legacy probe rounds (raw score scale)',
  'progress.erasNote': 'Scores from different objective eras are tagged; ratio-scale = sum_radii / 2.635 (raw scale = above 10).',
  'progress.ratioOnlyEmpty': 'No ratio-scale rounds (toggle above to include the legacy probes).',
  'progress.range': 'Plotted range',
  'progress.rangeFrom': 'From iteration',
  'progress.rangeTo': 'To iteration',
  'progress.rangeReset': 'Reset',
  'progress.championLineage': 'Champion lineage (nodes that held the best)',
  'progress.championHere': 'Champion at this iteration',
  'forest.star': 'Best node: {round} / {node}',
  'rounds.scaleRatio': 'ratio',
  'rounds.scaleRaw': 'raw·probe',
  'worlds': '{count} / {total}',
  'truncatedRounds': 'Some rounds are not listed (directory listing truncated; {count} read).',
  'truncatedDreams': 'Showing the latest {count} reports.',
} satisfies Record<DreamRsiKey, string>
